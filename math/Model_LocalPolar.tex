...
\begin{tikzpicture}[
  ...
  % --- position vector of the light centre, theta = (theta_1,theta_2) ----
  \draw[inkcol, thick, dashed, -{Latex[length=2.5mm]}] (Origin) -- (ImgPos);
  \node[inkcol, font=\sffamily, anchor=south] at ($(Origin)+(1.88,1.6)$) {$\boldsymbol{\theta}=(\theta_1,\theta_2)$};
  ...
  % --- small dot marking the light centre (local coordinate origin) ------
  \fill[inkcol] (ImgPos) circle (0.05);
  ...
  \node[anglecol, font=\sffamily\large, anchor=south] at ($(ImgPos)+(50:0.55)$) {$\tilde{\theta}$};
